 ...
 * @param {boolean=} options.showDetail 是否显示详情信息，比如包描述等信息，如果是，用列表形式展现
 * @return {Promise}
 */
function listInstalledComponents(options) {
    ...
    if (options.availableUpdate) {
        result = result.then(function (result) {
            ...
            return fetchAvailableUpdate(fetchUpdatePkgs).then(
                function () {
                    // 对于指定包的，获取其所有可用的版本信息
                    var listPkg = result.installedPkgs[0];
                    if (pkgName && listPkg) {
                        return listPkg.repos.fetchAllVersions();
                    }
                }
            ).then(
                function (allVersions) {
                    result.allVersions = allVersions;
                    return result;
                }
            );
        });
    ...
    return result.then(function (result) {
        ...
        if (options.showDetail) {
            print.listPackageDetail(result, options);
        }
        else {
