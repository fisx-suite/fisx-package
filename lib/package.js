 ...
 * @param {string} options.version 包的版本号
...
function Package(options) {
    ...
    this.initDependencies(this.dep);
}
...
Package.prototype.initRepository = function (endPoint) {
    ...
    if (!Repos) {
        throw new Error('unknown endpoint %s', endPoint.type);
    }
 ...
 */
Package.prototype.initDependencies = function (depMap) {
    this.dep = depMap;
    ...
    Object.keys(depMap || {}).forEach(function (name) {
        var pkg = new Package({
            name: name,
            version: depMap[name],
            endPoint: this.repos.getDepEndPoint()
        });
        deps.push(pkg);
    }, this);
...
Package.prototype.initInstallInfo = function (info) {
    ...
    this.initDependencies(info.dep);
...
function parseEndPointInfo(pkgStr, endPointType) {
    ...
    if (endPointType === REPOS_TYPE.LOCAL || endPointType === REPOS_TYPE.URL) {
        ...
        if (len > 1) {
            if (endPointType && endPointType !== REPOS_TYPE.GITHUB) {
                throw new Error(
                    util.format(
                        'parse endpoint %s value fail: %s, require github',
                        endPointType, pkgStr
                    )
                );
            }
            endPoint = {
                type: REPOS_TYPE.GITHUB,
...
Package.find = function (pkgName, pkgs, returnIndex) {
    var found;
    var result = pkgs.some(function (pkg, index) {
