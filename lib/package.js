 ...
 * @param {string} info.dir 安装的临时目录
 * @param {string} info.root 包安装的根目录
 * @param {string=} info.resolvedUrl 下载安装的 url
 */
...
function getPkgInstallSourceInfo(pkg) {
    ...
    var repos = pkg.repos;
    if (!repos.pkgVersion) {
        repos.pkgVersion = installVersion;
    }
 ...
 * @return {string}
 */
Package.prototype.getRepositoryUrl = function () {
    var metaData = this.metaData || {};
    var optionalRepoUrl = this.repos.getReposUrl();
    var repoUrl;
    ...
    repoUrl || (repoUrl = optionalRepoUrl);
    if (repoUrl) {
        repoUrl = repoUrl
            .replace(/^git\+/, '')
            .replace(/^git/, 'http');
