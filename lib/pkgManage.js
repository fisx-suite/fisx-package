...
function checkInstall(pkg, options, result) {
    ...
    debug('%s - %s : %s', oldVersion, installVersion, isSatisfy);
    var forceLatest = options.forceLatest;

    if (!isSatisfy || forceLatest) {
        var hasConflict = semver.isConflict(oldVersion, installVersion);
        debug('version %s vs %s is conflict: %s', oldVersion, installVersion, hasConflict);
        var isCurrentNewer;
        try {
            isCurrentNewer = semver.gtr(installVersion, oldVersion);
            debug('%s is newer than %s: %s', installVersion, oldVersion, isCurrentNewer);
        }
        catch (ex) {
            isCurrentNewer = true; // 如果版本号不规范，那就当当前要安装版本最新好了。。
            installVersion && debug('compare %s - %s: %s', installVersion, oldVersion, ex);
        }

        if ((hasConflict && forceLatest && isCurrentNewer)
            || (isCurrentNewer && !hasConflict)
        ) {
            // 如果要安装版本比当前已经安装版本老，则使用当前安装的版本
...
exports.install = function (pkg, options) {
    ...
    return Promise.try(function () {
        return checkInstall(pkg, options, {name: pkg.name, version: pkg.version});
