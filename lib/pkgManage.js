...
function uninstallPkg(pkg, options) {
    ...
    return removePkgs.reduce(function (previous, item) {
        ...
        var promise = previous.then(doneHandler).catch(function () {
            result.push(item);
        });
        promise.myId = item.toString();
        return promise;
...
exports.install = function (pkg, options) {
    ...
    return Promise.try(function () {
        ...
        function (result) {
            ...
            if (result.installed) {
                var expectVersion = result.version;
                pkg.version || (pkg.version = expectVersion);

                // 这里对于未安装过情况，是作为安装的版本，所以要重置下
                result.version = result.installVersion;

                // 如果冲突，则当前允许安装版本改成当前安装的版本
                result.conflict && (pkg.version = expectVersion);
            }
