 ...
 */
function updatePkgInfos(currPkgs, updatePkgs, ignoreNewPkg) {
    var toAddPkgs = [];
    updatePkgs.forEach(function (pkg) {
        ...
        for (var i = 0, len = currPkgs.length; i < len; i++) {
            ...
            if (pkgName === currItem.name) {
                ...
                if (!currItem.hasInstalled
                    && Package.getKey(currItem, false) === key
                ) {
                    // 增加镜像
                    debug('add mirror: %s -> %s', currItem, pkg);
                    currItem.mirror = pkg;
                }
                else if (!pkg.alreadyInstalled && pkg.newInstalled
...
exports.saveInstallInfo = function (installPkgs, options) {
    ...
    if (options.allDep && options.allDevDep) {
        ...
        updatePkgInfos(deps, installPkgs, true);
        updatePkgInfos(devDeps, installPkgs, true);
    }

    // 用指定的安装成功包的信息，替换依赖清单里定义的包的信息
    if (!options.allDep && !options.allDevDep) {
        if (saveToDep) {
            replacePkg(deps, installPkgs);
        }
        if (saveToDevDep) {
            replacePkg(devDeps, installPkgs);
        }
    }
