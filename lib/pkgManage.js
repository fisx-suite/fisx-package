...
function uninstallPkg(pkg, options) {
    ...
    var done = function (processPkg, uninstalledPkg) {
        ...
        if (processPkg) {
            return uninstall(processPkg, options.canUninstall, options.confirm);
        }
...
function checkInstall(pkg, options, result) {
    ...
    debug('%s - %s -> %s satisfy: %s', pkg, oldVersion, installVersion, isSatisfy);
...
exports.install = function (pkg, options) {
    ...
    var repos = pkg.repos;
    var resolved = !options.update && pkg.isResolved();
    ...
    return Promise.try(function () {
        ...
        return resolved || repos.fetchAvailableVersion(fetchVersion).then(
            function (result) {
        ...
        function (result) {
            ...
            if (result.installed) {
                ...
                if (!lockInfo || !lockInfo.resolved) {
                    logger.warn('missing installed package lock (resolved) info: %s, '
