...
function getPkgInstallTitle(expectedPkg, installPkg) {
    ...
    var pkgInfo = (config.defaultEndPoint.type !== installPkg.endPoint.type)
        ? (installPkg.endPoint.type + ':').yellow
        : '';
    ...
    pkgInfo += (pkgVersion ? (pkgName + '@' + pkgVersion).green : pkgName.green);
...
function listPkgAllVersionInfos(versionData, options) {
    ...
    var infos = [];
    var prefixIndent = getSpaceStr(1);
    ...
    logger.info('%s all available versions:', options.name.green);
 ...
 * @param {Object=} options 选项
 */
exports.listPackageDetail = function (info, options) {
    var projectPkg = project.getProjectPackage();
    var installedPkgs = info.installedPkgs || [];
    var notInstallManifestPkgs = info.notInstallManifestPkgs || [];
    var allPkgs = installedPkgs.concat(notInstallManifestPkgs);

    console.log('\nproject root: '. green + projectPkg.root + '\n');
    ...
    allPkgs.forEach(function (pkg) {
        ...
        if (pkg.installed) {
            ...
            homeUrl = metaData.homepage || metaData.repository;
        ...
        fields = formatUtil.alignValues(fields, 'left');

        console.log(fields[0].green + pkg.name + ' ' + errorInfo.red);
        if (pkg.installed) {
            console.log(fields[1].green + pkg.installVersion || '-');
            console.log(fields[2].green + (descr || '-').gray);
            console.log(fields[3].green + (homeUrl || '-').blue);
        }
        console.log('');
    });
};
