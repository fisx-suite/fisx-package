 ...
 */
function listPackageByList(pkg, options, listedMap) {
    var expectPkg = pkg;
    pkg = pkg.getRealPackage();
    ...
    if (pkg.isRoot) {
        ...
        var pkgVersion = pkgInfo.version;
        var type = (pkgInfo.type || '').yellow;
        var pkgInfoStr = type + pkgName + ' ' + errorInfo.red;

        var expectInfo = getExpectVersionInfo(pkgVersion, expectPkg);
        ...
        fields = formatUtil.alignColumn(fields, 'left', function (value) {
            return (getSpaceStr(4) + value + ' ').green;
        });

        console.log(fields[0] + pkgInfoStr);
        if (pkg.installed) {
            var updateInfo = getPkgUpdateInfo(pkg);
            console.log(
                fields[1] + (pkg.installVersion || '-')
                + ' ' + [expectInfo, updateInfo].join(' ')
            );
            descr && console.log(fields[2] + descr.gray);
            homeUrl && console.log(fields[3] + homeUrl);
            repoUrl && console.log(fields[4] + repoUrl);

            deps = pkg.getDependencies();
            var depInfos = [];
            deps.forEach(function (item, index) {
                depInfos[index] = getPkgInstallTitle(null, item.getRealPackage());
            });
            depInfos.length && console.log(fields[5] + depInfos.join(', '));
        }
        console.log('');
    }
    ...
    deps.forEach(function (item) {
        listPackageByList(item, options, listedMap);
    });
}
...
exports.listPackages = function (info, options) {
    ...
    switch (options.style) {
        case 'list':
            listPackageByList(projectPkg, options, {});
            break;
        default:
