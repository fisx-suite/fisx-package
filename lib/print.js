...
function getPkgInstallTitle(pkg) {
    var pkgInfo = pkg.endPoint.type + ':' + pkg.name;
    var pkgVersion = pkg.installVersion;
    pkgVersion && (pkgInfo += '@' + pkgVersion);
    return pkgInfo;
}

function getInstallCmdPkgInfo(pkg, option) {
    var pkgInfo = getPkgInstallTitle(pkg);
...
function getListCmdPkgInfo(pkg, option) {
    ...
    if (deep) {
        pkgInfo += getPkgInstallTitle(pkg);
    }
    else {
        // 项目包信息
...
function initPkgInstallInfo(pkg, option) {
    ...
    var deep = option.deep;
    var pkgInfo = getSpaceStr(deep * option.indent);

    pkgInfo += (option.isLast ? '└── ' : '├── ');
    pkgInfo += (option.ls
        ? getListCmdPkgInfo(pkg, option)
        : getInstallCmdPkgInfo(pkg, option));
