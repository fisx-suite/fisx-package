...
exports.listPackageDetail = function (info, options) {
    ...
    logger.info('project root: '. green + projectPkg.root + '\n');
    ...
    allPkgs.forEach(function (pkg) {
        ...
        console.log(fields[0] + pkg.name + ' ' + errorInfo.red);
        if (pkg.installed) {
            console.log(fields[1] + pkg.installVersion || '-');
            console.log(fields[2] + (descr || '-').gray);
            console.log(fields[3] + (homeUrl || '-'));
            console.log(fields[4] + (repoUrl || '-'));
        }
        console.log('');
    });
