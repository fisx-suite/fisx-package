...
function initPkgInfo(metaData, pkgInfo) {
    if (pkgInfo) {
        pkgInfo.dep = metaData.dependencies;
    }
    else {
...
/**
 * 将安装的包的信息转成 `require.config` 的配置信息，供项目开发读取使用
 *
 * @param {Array.<Package>} pkgArr 依赖包数组
 * @param {string} base 要计算的基础路径
 * @return {Array.<Object>}
 */
exports.toRequireConfig = function (pkgArr, base) {
    var pkgConfs = [];
    pkgArr.forEach(function (pkg, index) {
...
exports.saveManifestInfo = function (deps, devDeps) {
    ...
    if (deps) {
        ...
        var requireConfig = this.toRequireConfig(deps, path.resolve(this.root, baseUrl));
        info[config.moduleConfigKey] = requireConfig;
    }

    fs.writeFileSync(manifest.file, JSON.stringify(data, null, 2), 'UTF-8');
