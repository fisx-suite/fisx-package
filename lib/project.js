 ...
 *
 * @inner
 * @param {string} targetDir 保存目标目录
 * @param {Object} endPoint endpoint 信息
 */
function saveEndpointInfo(targetDir, endPoint) {
    fs.writeFileSync(getEndpointFile(targetDir), JSON.stringify(endPoint, null, 2));
}
 ...
 *
 * @inner
 * @param {string} targetDir 读取的目标目录
 * @return {?Object}
 */
function readEndpointInfo(targetDir) {
    var file = getEndpointFile(targetDir);
    ...
    files.forEach(function (fileName) {
        ...
        if (stat.isDirectory()) {
            ...
            if (definedPkg) {
                ...
                pkgInfo.endPoint = readEndpointInfo(pkgDir)
                    || definedPkg.endPoint;
            }
...
exports.addInstalledPkg = function (pkg, tempPkgDir) {
    ...
    // 保存 endpoint 信息
    saveEndpointInfo(targetDir, pkg.endPoint);
...
exports.toModulePackageConfig = function (pkgArr, base) {
    ...
    pkgArr.forEach(function (pkg) {
        ...
        pkgConfs.push({
            name: rawPkg.aliasName || pkg.name,
            location: location,
            main: main
        });
    }, this);
 ...
 * 获取依赖的 endpoint 信息
 *
 * @param {Object} depMap 依赖信息
 * @return {Object}
 */
exports.getDepEndPoints = function (depMap) {
    var endpointMap = {};
    Object.keys(depMap).forEach(function (name) {
        var pkg = this.findPkgFromManifest(name);
        if (pkg) {
            endpointMap[name] = pkg.endPoint;
        }
    }, this);
    return endpointMap;
};
...
exports.readPackageManifest = function (pkgDir, pkgRoot, findupBottom) {
    var pkgInfo = readPkgInfo(pkgDir, pkgRoot, findupBottom);
    if (pkgInfo.dep) {
        pkgInfo.depEndPoints = this.getDepEndPoints(pkgInfo.dep);
    }
    return pkgInfo;
};
