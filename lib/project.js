...
function updateModulePackageConfig(rawPkgConf, allPkgArr, removePkgNames, updatePkgs, basePath) {
    ...
    allPkgArr.forEach(function (pkg) {
        var rawPkg = pkg;
        console.log('install: %s', pkg.name)
        ...
        // 如果已经存在且没有更新则跳过
        var existedIndex = findPkgModuleConf(rawPkgConf, pkg.name, basePath);
