...
exports.cacheReposInfo = function (info, persistence) {
    debug('ready cache repo info: %j', info);
    ...
    debug('cache key: %s => %s', info.uri, cacheKey);
...
function clearComponentCache(uri, isComponent) {
    var cacheData = exports.getReposCacheDetail(isComponent);
    var cacheKey = getReposCacheKey(uri);
    var removeVersions = cacheData[cacheKey] || [];
    delete cacheData[cacheKey];

    debug('clear component %s: %s', uri, cacheKey);
    removeVersions.forEach(function (item) {
        // 删除文件
        helper.rmFile(item.file);
        debug('rm cache: %j', item);
    });
}
 ...
 * @param {Object=} options.components 要清空的组件，可选
 *        结构为：{isComponent: true, uris: ['npm:jquery']}
 */
exports.clearReposCache = function (options) {
    debug('clear repos cache: %j', options);
    ...
    if (options.all) {
        ...
        if (components) {
            var isComponent = components.isComponent;
            var toRemoveComponentUris = components.uris || [];
            toRemoveComponentUris.forEach(function (item) {
                if (isComponent == null) {
                    clearComponentCache(item, true);
                    clearComponentCache(item, false);
                }
                else {
                    clearComponentCache(item, isComponent);
                }
            });
        }
    }
