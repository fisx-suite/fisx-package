 ...
 * @param {string} target 下载缓存的文件
 * @param {Function} cb 下载完成执行的回调
 */
function startDownload(emitter, url, target, cb) {
    var size;
    var type;
    var receiveSize = 0;

    logger.info('request %s%s...', url, target ? (', cache to: ' + target) : '');

    var downloadStream = request.get({
            url: url,
            gzip: true,
            headers: {
                'User-Agent': 'request'
        ...
        function (err, res, body) {
            ...
            if (err || parseInt(statusCode / 100, 10) !== 2) {
                err || (err = 'response with statuscode ' + statusCode);
            }
 ...
 */
function downloadDone(emitter, cb, err) {
    if (emitter.hasErrorHappen) {
        return;
    }

    if (err) {
        logger.warn('request error happen: %s', err);
...
function readCacheDone(option, err, data) {
    ...
    var tempFile = option.tempFile;
    var url = option.url;
    var doneHandler = option.doneHandler;

    if (err) {
        return startDownload(emitter, url, tempFile, doneHandler);
    }
...
function download(url, option, cb) {
    ...
    var doneHandler = downloadDone.bind(this, emitter, cb);
    ...
    if (target) {
        ...
        emitter.writeDone = true;
        startDownload(emitter, url, null, doneHandler);
    }
