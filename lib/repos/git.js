...
GitRepos.prototype.fetchAvailableTags = function () {
    ...
    if (me._allTags) {
        return Promise.resolve(me._allTags);
    }
...
GitRepos.prototype.fetchAvailableBranch = function () {
    ...
    if (me._allBranches) {
        return Promise.resolve(me._allBranches);
    }
    ...
    return fetchAvailableVersions(me, me.allBranchUrl).then(
        function (data) {
            data = data.map(function (item) {
                ...
                var filterTags = ['gh-pages'];
                return filterTags.indexOf(version) === -1;
            });
