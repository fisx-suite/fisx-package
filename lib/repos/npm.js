...
function NPMRepos(options) {
    ...
    pkgName && (this.metaDataUrl = this.registry + '/' + encodePkgName(pkgName));
}
...
NPMRepos.prototype.fetchVersionMetaData = function (option) {
    ...
    var promise;
    if (me._allVersionMetaData) {
        var versionInfo = me._allVersionMetaData.versionData[version];
        promise = versionInfo
            ? Promise.resolve(versionInfo)
            : Promise.reject('given version: ' + version + ' not found');
    }
    else {
        promise = me.fetch.json([
            me.registry, encodePkgName(me.pkgName), version
        ].join('/'));
    }
 ...
 * @return {Promise}
 */
NPMRepos.prototype.fetchAvailableVersion = function (fetchVersion) {
    var me = this;
    fetchVersion || (fetchVersion = me.pkgVersion);
    me.debug('fetch available version info: %s...', fetchVersion);

    return me.fetchAllVersions.apply(me, arguments).then(
        function (data) {
            me.debug('fetch the version %s to download...', fetchVersion);

            var result = me.getFetchVersion(
                fetchVersion, data.versions, data.tags
            );
            if (result.err) {
 ...
 * @return {Promise}
 */
NPMRepos.prototype.fetchAllVersions = function () {
    var me = this;
    me.debug('fetch all version info...');

    if (me._allVersionMetaData) {
        return Promise.resolve(me._allVersionMetaData);
    }

    return Repos.prototype.fetchAvailableVersion.apply(me, arguments).then(
        function (data) {
            me._reposMetaData = data;

            var result = getAvailableVersionInfo(data);
            me.debug('fetch all version info done: %j, %j', result.allTags, result.allVersions);
            return (me._allVersionMetaData = {
                tags: result.allTags,
                versions: result.allVersions,
                tagVersionMap: result.tagMap,
                versionData: result.versionMap
            });
        }
    );
};
 ...
 * @return {?Object}
 */
NPMRepos.prototype.fetchReposMetaData = function () {
    var me = this;
    return this.fetchAvailableVersion().then(function () {
        return me._reposMetaData;
    });
};
